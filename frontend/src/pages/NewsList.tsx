    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <Space wrap size="middle" style={{ width: '100%' }}>
          ...
          />
          <Select value={searchParams.get('category') || 'all'} style={{ minWidth: 180 }} onChange={handleCategoryChange}>
          ...
          <Select value={searchParams.get('sortBy') || 'publishedAt'} style={{ minWidth: 180 }} onChange={handleSortChange}>
            <Option value="publishedAt">🕒 По дате</Option>
            <Option value="views">👁 По просмотрам</Option>
            <Option value="likes">❤️ По лайкам</Option>
          </Select>
          <Select value={searchParams.get('isAiGenerated') || 'all'} style={{ minWidth: 180 }} onChange={handleAiFilterChange}>
            <Option value="all">📋 Все новости</Option>
            <Option value="true">🤖 AI-рерайт</Option>
            <Option value="false">📄 Оригиналы</Option>
          </Select>
          {hasActiveFilters && (
            <Button icon={<ClearOutlined />} onClick={handleClearFilters} size="small">Сбросить фильтры</Button>
          )}
        </Space>

        {hasActiveFilters && (
          <Text type="secondary" style={{ display: 'block', marginTop: 8 }}>Показаны результаты с фильтрами</Text>
        )}
      </div>

      <Divider style={{ margin: '12px 0' }} />

      <Spin spinning={isLoading} description="Загрузка новостей...">
        {error ? (
          <Empty description={<Text type="danger">Ошибка загрузки: {error}</Text>}>
            <Space>
                ...
                <Space align="start" style={{ width: '100%', justifyContent: 'space-between' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <Space wrap size={[8, 8]}>
                      ...
                      {item.author && <Text type="secondary" style={{ fontSize: '12px' }}>• {item.author}</Text>}
                    </Space>
                  </div>
                  <Space size="middle" style={{ flexShrink: 0, marginLeft: 16 }}>
                    <Tooltip title="Просмотры">
                      <Text type="secondary" style={{ fontSize: '13px' }}><EyeOutlined /> {item.views || 0}</Text>
                    </Tooltip>
                    <Tooltip title="Лайки">
                      <Text type="secondary" style={{ fontSize: '13px' }}><HeartOutlined /> {item.likes || 0}</Text>
                    </Tooltip>
                  </Space>
                </Space>
              </div>
          ...
          <Empty
            description={hasActiveFilters ? 'Новости по вашему запросу не найдены' : 'Новости пока не добавлены'}
            style={{ padding: '40px 0' }}
          >
            {hasActiveFilters ? (
              <Button onClick={handleClearFilters}>Сбросить фильтры и показать все</Button>
            ) : (
              <Button onClick={() => fetchNews()}>Обновить список</Button>
            )}
          </Empty>
        )}
      </Spin>
