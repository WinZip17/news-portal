...
const NewsManagement: React.FC = () => {
  ...
  useEffect(() => { loadNews(); }, [statusFilter]);
  ...
  const loadNews = async () => {
    ...
    } catch (error) { message.error('Ошибка загрузки'); }
    setLoading(false);
  };
  ...
  const columns: ColumnsType<News> = [
    ...
    { title: 'Тип', dataIndex: 'isAiGenerated', key: 'type', width: 100,
      render: (ai) => ai ? <Tag icon={<RobotOutlined />} color="blue">AI</Tag> : <Tag color="green">Пользователь</Tag> },
          ...
          {record.status === NewsStatus.PENDING && (
            <>
              <Button type="primary" size="small" icon={<CheckCircleOutlined />}
                      onClick={() => handleModerate(record.id, NewsStatus.PUBLISHED)}>Опубликовать</Button>
              <Button danger size="small" icon={<CloseCircleOutlined />}
                      onClick={() => handleModerate(record.id, NewsStatus.REJECTED)}>Отклонить</Button>
            </>
          )}
          {record.status === NewsStatus.PUBLISHED && (
            <>
              <Button size="small" onClick={() => handleModerate(record.id, NewsStatus.ARCHIVED)}>В архив</Button>
              <Button size="small" onClick={() => handleModerate(record.id, NewsStatus.PENDING)}>На модерацию</Button>
            </>
          )}
          {record.status === NewsStatus.ARCHIVED && (
            <>
              <Button size="small" icon={<UndoOutlined />}
                      onClick={() => handleModerate(record.id, NewsStatus.PUBLISHED)}>Восстановить</Button>
  ...
  const tabItems = [
    ...
    { key: NewsStatus.ARCHIVED, label: <span><InboxOutlined /> Архив</span> },
  ];
