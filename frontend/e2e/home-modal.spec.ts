...
test.describe('Home → news modal', () => {
  ...
  test('opens news detail modal from home page card', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '📰 Short News' })).toBeVisible();
    ...
    await expect(modal.getByRole('heading', { name: mockNewsItem.title })).toBeVisible({ timeout: 15_000 });
    await expect(modal.getByText(mockNewsItem.summary)).toBeVisible();
  });

  test('opens modal from deep link ?news=id', async ({ page }) => {
    await page.goto(`/?news=${mockNewsItem.id}`);
